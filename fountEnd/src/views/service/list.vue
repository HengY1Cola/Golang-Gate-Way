<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <router-link :to="'/service/serviceCreateHttp/'">
        <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit">
          添加HTTP服务
        </el-button>
      </router-link>
      <router-link :to="'/service/serviceCreateTcp/'">
        <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit">
          添加TCP服务
        </el-button>
      </router-link>
      <router-link :to="'/service/serviceCreateGrpc/'">
        <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit">
          添加GRPC服务
        </el-button>
      </router-link>
    ...
    >
      ...
      <el-table-column label="节点数" min-width="60px">
        <template slot-scope="{row}">
          <span>{{ row.totalNode }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="{row,$index}">
          <router-link :to="'/service/serviceStat/'+row.id">
            <el-button type="primary" size="mini" style="margin-right: 10px;">
              统计
            </el-button>
          </router-link>
          <router-link v-if="row.load_type===0" :to="'/service/serviceEditHttp/'+row.id">
            <el-button type="primary" size="mini" style="margin-right: 10px;">
              修改
            </el-button>
          </router-link>
          <router-link v-if="row.load_type===1" :to="'/service/serviceEditTcp/'+row.id">
            <el-button type="primary" size="mini" style="margin-right: 10px;">
              修改
            </el-button>
          </router-link>
          <router-link v-if="row.load_type===2" :to="'/service/serviceEditGrpc/'+row.id">
            <el-button type="primary" size="mini" style="margin-right: 10px;">
              修改
            </el-button>
          </router-link>
          <el-button size="mini" type="danger" @click="handleDelete(row,$index)" style="margin-right: 10px;">
            删除
          </el-button>
        </template>
      </el-table-column>
